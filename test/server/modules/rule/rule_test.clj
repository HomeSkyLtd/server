...
(deftest test-app
  ...
  (testing "new node without nodeId"
    (let [obj {:rules [{:commandId 1 :value 20 :clauses [{:lhs "1.1", :operator ">", :rhs "20"}]}]}
            houseId 1]
      (is (= (rule/new-rules obj houseId) {:status 400 :errorMessage "Define nodeId, commandId, value and clauses."}))))

  (testing "new node without commandId"
    (let [obj {:rules [{:nodeId 1 :value 20 :clauses [{:lhs "1.1", :operator ">", :rhs "20"}]}]}
            houseId 1]
      (is (= (rule/new-rules obj houseId) {:status 400 :errorMessage "Define nodeId, commandId, value and clauses."}))))

  (testing "new node without value"
    (let [obj {:rules [{:nodeId 1 :commandId 1 :clauses [{:lhs "1.1", :operator ">", :rhs "20"}]}]}
            houseId 1]
      (is (= (rule/new-rules obj houseId) {:status 400 :errorMessage "Define nodeId, commandId, value and clauses."}))))

  (testing "new node without clauses"
    (let [obj {:rules [{:nodeId 1 :commandId 1 :value 20}]}
            houseId 1]
      (is (= (rule/new-rules obj houseId) {:status 400 :errorMessage "Define nodeId, commandId, value and clauses."})))))